# 시나리오
# 초기상태 : FIND_WALL
# 전방 거리 4.0m 이상일 경우 좌회전, 이하일 경우 벽 발견 및 상태 변경(MOVE_TO_WALL)
# 상태 : MOVE_TO_WALL
# 전방 거리 0.5m 이상일 경우 천천히 전진, 이하일 경우 정지 및 상태 변경(FIND_NEXT_WALL)
# 상태 : FIND_NEXT_WALL
# 다음 벽을 찾기 위한 좌회전, 전방 거리 4.0m 이상일 경우 상태 변경(FIND_WALL)
